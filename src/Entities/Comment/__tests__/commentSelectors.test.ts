...
describe("Comment Selectors", () => {
  ...
  it("Should Comments.items", () => {
    expect(selectors.getAllComments(testState)).toEqual(testState.Comments.items);
  });
  ...
  it("Should Comments.loading", () => {
    expect(selectors.getLoadingStatus(testState)).toEqual(testState.Comments.loading);
  });
